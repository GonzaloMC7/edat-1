...
int usuario_mas(char* scrn, char* name){
    ...
    /*Busca algún usuario con el mismo nombre, si lo encuentra (también cuentan los usuarios borrados) no permite continuar*/
    sprintf(query, "SELECT usuario.scrname FROM usuario WHERE usuario.scrname='%s';", scrn);
    SQLExecDirect(stmt, (SQLCHAR*) query, SQL_NTS);
    SQLBindCol(stmt, 1, SQL_C_CHAR, scrname, sizeof(scrname), NULL);    
    ret = SQLFetch(stmt);
    if (SQL_SUCCEEDED(ret)) {
    ...
    /*Encuentra el mayor id de usuario*/
    ...
    /*Crea un usuario con el id anterior incrementado en 1, los datos introducidos y la fecha actual*/
    sprintf(query, "INSERT INTO usuario(idusuario,nombre,ccard,scrname,joindate,expdate,exists) VALUES(%d, '%s', NULL, '%s', '%s', '10000-1-1', '1');", idusuario+1, name, scrn, fecha );
...
int usuario_menos(char* scrn){
    ...
    /*Busca el usuario introducido, si no lo encuentra no permite seguir*/
    ...
    /*Modifica su campo exists para que no sea mostrado, pero no lo borra de la base de datos*/
